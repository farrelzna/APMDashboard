...
const ProjectStatistics = defineAsyncComponent(() =>
  import("@/components/dashboards/ProjectStatistics.vue")
);
...
const summaryMetrics = computed(() =>
  datas.value.filter((d) =>
    ["Total Client", "Total Project", "Total User"].includes(d.title)
  )
);
const teamMetrics = computed(() =>
  datas.value.filter((d) =>
    ["Engineer Internal", "Engineer External"].includes(d.title)
  )
);
...
const importantProject = computed(() => {
  ...
  for (let i = 1; i < dashboardData.value.projects.length; i++) {
    if (
      new Date(dashboardData.value.projects[i].end_date) <
      new Date(earliest.end_date)
    ) {
...
const openProject = async (projectId) => {
  ...
  try {
    ...
    const progress = {
      task_total: Array.isArray(data?.project_status)
        ? data.project_status.length
        : 0,
      task_complete: Array.isArray(data?.project_status)
        ? data.project_status.filter(
            (s) => (s.status || "").toLowerCase() === "complete"
          ).length
        : 0,
    };
...
<template>
  <div v-if="hasDashboardData" class="w-full flex flex-col gap-4">
    <!-- Banner -->
    <div
      class="bg-[#Ff5f00] rounded-xl p-8 shadow-sm position-relative overflow-hidden"
    >
      <!-- Background Image -->
      <img
      ...
      <div
        class="d-flex align-end justify-space-between position-relative"
        style="z-index: 1"
      >
        <div class="max-w-2xl">
          <div class="text-white/80 text-sm font-semibold mb-2">
            Dasa Aprindo Sentosa
          </div>
          ...
          <NuxtLink to="/dashboard/project">
            <button
              class="bg-black text-white rounded-xl text-sm font-medium px-6 py-2 scale-100 hover:scale-105 transition-transform duration-200 flex items-center"
            >
              New Project
        ...
        <div class="d-flex align-center">
          <div class="flex ga-12 items-center uppercase">
            <div class="d-flex align-center ga-2">
              <span
                class="text-white/70 font-bold tracking-widest hover:text-white/80 transition-colors duration-200"
                >Build</span
              >
            </div>
            <div class="d-flex align-center ga-2">
    ...
    <v-row class="gy-4" style="margin: 0 !important">
      ...
      <v-col cols="12" md="7" class="flex flex-col gap-4">
        ...
        >
          <div class="flex items-center justify-between w-full">
            <div class="flex items-center gap-4 min-w-0">
              <div
                size="56"
                class="bg-red-100 rounded-xl flex justify-center items-center w-14 h-14"
              >
                <v-icon icon="mdi-calendar-alert" color="error" />
              </div>
              <div class="flex flex-col truncate">
                <span class="text-[11px] text-gray-500 leading-4"
                  >Last/Due/Important Project</span
                >
                <h3 class="font-bold text-lg leading-5 truncate uppercase">
                  {{ importantProject.name }}
                </h3>
                <span
                  class="text-xs text-gray-500 uppercase tracking-wide truncate"
                  >{{ importantProject.customer_str.name }}</span
                >
              </div>
            </div>
            <div class="flex flex-col items-end gap-2 text-right">
              <span class="text-xs text-gray-600"
                >Due date {{ importantProject.end_date }}</span
              >
              <v-chip
                size="small"
                class="text-body-2"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          ...
          <div class="flex flex-col h-full">
            <v-card
              class="col-span-5 h-full flex flex-col"
              elevation="0"
              rounded="lg"
              style="border: 1px solid #f4f4f4"
            >
              <v-row no-gutters>
                <v-col cols="12" class="mb-2"> </v-col>
          ...
          <div class="flex flex-col gap-4 h-full">
            <!-- summary -->
            <v-card
              class="col-span-7 pa-5 rounded-2xl flex-1"
              elevation="0"
              rounded="lg"
              style="border: 1px solid #f4f4f4"
            >
        ...
        >
          ...
          <div
            class="pa-4 d-flex flex-column ga-3 position-relative"
            style="max-height: 400px; overflow: hidden"
          >
            <div
              class="engineer-workload-scroll"
              ref="scrollWorkload"
                  ...
                  <div class="d-flex align-center ga-3">
                    ...
                    <div>
                      <div class="text-body-1 font-weight-semibold">
                        {{ eng.engineer.full_name }}
                      </div>
                      <div class="text-caption text-grey-darken-1">
                        {{ eng.engineer.job_titles }}
                      </div>
                    </div>
                  </div>
            ...
            <div
              v-if="
                dashboardData.engineers && dashboardData.engineers.length > 3
              "
              style="
                position: absolute;
                bottom: 0;
      ...
      <v-col cols="12" md="5" class="d-flex flex-column md:pr-0">
        ...
        >
          <div
            class="pa-4 pb-3 d-flex align-center justify-space-between flex-wrap ga-3"
          >
            <h5 class="text-subtitle-1 text-md-h6 font-weight-semibold">
              Project list
            </h5>
            <v-btn-toggle
          ...
          >
            ...
            >
              ...
              >
                ...
                <div
                  class="d-flex flex-column flex-sm-row align-start align-sm-center justify-space-between mb-3 ga-2"
                >
                    ...
                    <v-avatar
                      ...
                      :image="
                        project.pm_str.photo
                          ? config.public.apiMedia + '/' + project.pm_str.photo
                ...
                <div class="mb-4">
                  <div
                    class="text-caption font-weight-medium text-grey-darken-2 mb-3"
                  >
                    Finance:
                  </div>
                  <div class="d-flex flex-column ga-3">
                    ...
                    >
                      ...
                      ></div>
                      <div
                        class="d-flex flex-column flex-sm-row align-start align-sm-center justify-space-between flex-grow-1 ga-1"
                      >
                    ...
                    <div
                      v-if="!project.finances || project.finances.length === 0"
                      class="text-caption text-grey-darken-1 pl-5"
                    >
                      No finance data
                    </div>
                  </div>
                ...
                <div
                  v-if="project.project_status && project.project_status.length"
                >
                  <div
                    class="text-caption font-weight-medium text-grey-darken-2 mb-3"
                  >
                  ...
                  <div class="d-flex flex-column ga-3">
                    <div
                      ...
                      :key="
                        status.id || status.description + status.date_submit
                      "
                      class="d-flex align-center ga-3"
                    >
                      <div
            ...
            <div
              ...
              class="text-center pa-8 text-body-2 text-grey"
            >
              No project found
            </div>
          </div>
